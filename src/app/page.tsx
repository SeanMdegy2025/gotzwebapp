            ...
            <div className="grid gap-8 sm:gap-10 lg:grid-cols-[1.05fr_1fr] lg:gap-16">
              <div className="flex flex-col space-y-6 sm:space-y-8 order-2 lg:order-1">
                <div className="grid gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-1">
                  {(() => {
                    const grouped = new Map<string, Array<{ value: string; detail?: string }>>();
                    const groupOrder: string[] = [];
                    for (const ch of contactChannels) {
                      const label = ch.label.trim() || "Contact";
                      if (!grouped.has(label)) {
                        grouped.set(label, []);
                        groupOrder.push(label);
                      }
                      grouped.get(label)!.push({ value: ch.value, detail: ch.detail });
                    }
                    return groupOrder.map((label) => (
                      ...
                      >
                        <p className="mb-3 text-xs font-bold uppercase tracking-[0.35em] text-safari-gold">{label}</p>
                        <div className="mb-auto space-y-4">
                          {grouped.get(label)!.map((entry, i) => (
                            <div key={`${entry.value}-${i}`}>
                              <p className="text-lg sm:text-xl md:text-2xl font-heading font-bold leading-tight text-white break-all">{entry.value}</p>
                              {entry.detail ? <p className="mt-1 text-sm uppercase tracking-[0.2em] leading-relaxed text-white/70">{entry.detail}</p> : null}
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      <footer className="relative bg-gradient-to-b from-white via-safari-sand/20 to-white border-t border-safari-sand/30">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8 lg:py-10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center justify-center gap-2 sm:gap-4 pr-14 sm:pr-16 md:pr-20">
              <Link href="https://www.instagram.com/gotanzaniasafariltd?utm_source=qr&igsh=MWJtYXFmMWFhbTQyYg==" target="_blank" rel="noopener noreferrer" className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded transition-all duration-300 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-safari-gold focus:ring-offset-2" aria-label="Instagram">
                <svg className="h-6 w-6 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden>
                  <defs>
                  ...
                  <rect x="0" y="0" width="24" height="24" rx="6" fill="url(#instagramGradient)" />
                  <circle cx="12" cy="12" r="5" fill="none" stroke="white" strokeWidth="2" />
                  <circle cx="18" cy="6" r="1.2" fill="white" />
                  <rect x="3.5" y="3.5" width="17" height="17" rx="5" ry="5" fill="none" stroke="white" strokeWidth="2" />
                </svg>
              </Link>
              <Link href="https://www.facebook.com/share/1cBDnPm7jQ/" target="_blank" rel="noopener noreferrer" className="min-h-[44px] min-w-[44px] inline-flex items-center justify-center rounded transition-all duration-300 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-safari-gold focus:ring-offset-2" aria-label="Facebook">
