...
export default function AdminLodgesEditPage() {
  const params = useParams();
  const router = useRouter();
  const id = Number(params.id);
  const [lodge, setLodge] = useState<AdminLodge | null>(null);
    ...
    getAdminLodge(id)
      .then((r) => {
        ...
        setForm({
          ...
          slug: l.slug || "",
          location: l.location || "",
          type: (l.type as "lodge" | "camp") || "lodge",
          mood: l.mood || "",
          short_description: l.short_description || "",
          description: l.description || "",
          image_base64: l.image_base64 || null,
          ...
          published_at: l.published_at ? l.published_at.split("T")[0] : "",
        });
        setImagePreview(l.image_base64 || null);
      })
      .catch(() => setLodge(null))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateAdminLodge(lodge.id, payload)
  ...
  if (!lodge) {
    return (
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <p className="text-gray-500">Lodge not found.</p>
        <Link href="/admin/lodges" className="mt-2 text-sm text-safari-green hover:underline">
          ← Back to Lodges
        </Link>
      </div>
    );
