...
export default function AdminAboutHighlightsPage() {
  ...
  const load = () => {
    setLoading(true);
    getAdminAboutHighlights()
      .then((r) => setHighlights(r.about_highlights))
      .catch(() => setHighlights([]))
    ...
    <>
      <header className="sticky top-16 z-10 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
        <div className="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">About Highlights</h1>
            <p className="mt-1 text-sm text-gray-500">
              Manage the highlight cards in the About section
            </p>
          </div>
          <Link
            href="/admin/about-highlights/create"
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {highlights.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="line-clamp-2 text-sm text-gray-500">{item.copy}</div>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                      <Link
                        href={`/admin/about-highlights/${item.id}/edit`}
                ...
                {highlights.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center">
                      <p className="text-sm font-medium text-gray-900">No highlights</p>
                      <p className="mt-2 text-sm text-gray-500">Get started by creating a new highlight.</p>
                      <div className="mt-6">
                        <Link
                          href="/admin/about-highlights/create"
                          className="inline-flex items-center gap-2 rounded-lg bg-safari-green px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-safari-green/90"
