...
import { adminGetAboutHighlight, adminUpdateAboutHighlight, adminDeleteAboutHighlight } from "@/lib/db/queries";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await adminGetAboutHighlight(id);
  if (!row) return NextResponse.json({ message: "Not found" }, { status: 404 });
  return NextResponse.json({ about_highlight: mapHighlight(row) });
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await adminUpdateAboutHighlight(id, body);
  const row = await adminGetAboutHighlight(id);
  if (!row) return NextResponse.json({ message: "Not found" }, { status: 404 });
  return NextResponse.json({ about_highlight: mapHighlight(row) });
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id) return NextResponse.json({ message: "Invalid id" }, { status: 400 });
  if (!hasDb()) return NextResponse.json({ message: "Not implemented" }, { status: 501 });
  await adminDeleteAboutHighlight(id);
